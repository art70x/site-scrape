...
<template>
  <div class="mt-6">
    <TransitionGroup v-if="history.length" tag="ul" name="fade" class="space-y-2">
      <li v-for="url in history" :key="url">
        ...
        >
          <Icon name="ri:history-line" class="text-2xl text-gray-700" />
          {{ url }}
        </button>
      </li>
    </TransitionGroup>
    <div v-else class="flex flex-col items-center justify-center py-8 text-center">
      <div class="mb-4 rounded-full bg-gray-200 p-4">
        <Icon name="ri:history-line" class="text-[48px] text-foreground/70" />
      </div>
      <h3>No history yet</h3>
      <p class="max-w-xs text-sm text-foreground/60">
